...
define( 'WC_AUTHNET_VERSION', '5.0.3' );
...
class WC_Authnet
{
    ...
    public function submenu_page()
    {
        ...
        <div class="wrap">
            ...
            <h3><?php 
        _e( 'About Authorize.net', 'wc-authnet' );
        ?></h3>
            <p><?php 
        printf( __( 'As a leading payment gateway, %sAuthorize.Net%s is trusted by more than 430,000 merchants, handling more than 1 billion transactions and $149 billion in payments every year. Authorize.Net has been working with merchants and small businesses since 1996 and will offer you a credit card payment solution that works for your business and lets you focus on what you love best.', 'wc-authnet' ), '<a href="https://reseller.authorize.net/application/?resellerId=100678" target="_blank">', '</a>' );
        ?></p>
            <h3><?php 
        _e( 'About this WooCommerce Extension', 'wc-authnet' );
        ?></h3>
            <p><?php 
        _e( 'This extension enables you to use the Authorize.net payment gateway to accept payments via credit cards directly on checkout on your WooCommerce powered WordPress e-commerce website without redirecting customers away to the gateway website.', 'wc-authnet' );
        ?> </p>
            <p><a class="button" href="<?php 
        echo  $this->settings_url() ;
        ?>"><?php 
        _e( 'Settings', 'wc-authnet' );
        ?></a>  <a class="button" href="<?php 
        echo  wc_authnet_fs()->contact_url() ;
        ?>"><?php 
        _e( 'Support', 'wc-authnet' );
        ?></a></p>
            <?php 
        ?>
                <h3><?php 
        _e( 'License', 'wc-authnet' );
        ?></h3>
                <?php 
        ?>
                    <p><?php 
        printf( __( 'You are using our %1$sFREE PRO%2$s version of the extension. Here are the features you will get access to if you upgrade to the %1$sENTERPRISE%2$s version:', 'wc-authnet' ), '<strong>', '</strong>' );
        ?></p>
                    <ol>
                        <li><strong><?php 
        _e( 'Process Subscriptions:', 'wc-authnet' );
        ?></strong> <?php 
        printf(
            __( 'Use with %1$sWooCommerce Subscriptions%2$s extension to %3$screate and manage products with recurring payments%4$s — payments that will give you residual revenue you can track and count on.', 'wc-authnet' ),
        ...
        echo  wc_authnet_fs()->get_upgrade_url() ;
        ?>"><strong><?php 
        echo  $upgrade_label ;
        ?></strong></a>
                    <?php 
        ...
        if ( $show_account ) {
            ...
            echo  wc_authnet_fs()->get_account_url() ;
            ?>"><?php 
            _e( 'Account', 'wc-authnet' );
            ?></a>
                    <?php 
        }
    ...
    public function capture_payment( $order_id )
    {
        ...
        WC_Authnet_API::log( "Info: Begin capturing payment for order {$order_id} for the amount of {$order->get_total()}" );
        ...
        if ( $order->get_payment_method() == 'authnet' ) {
            ...
            if ( $charge && $captured == 'no' ) {
                ...
                $transactionRequestType->setRefTransId( $order->get_transaction_id() );
                ...
                $complete_message = sprintf( __( 'Authorize.Net charge complete (Charge ID: %s)', 'wc-authnet' ), $trx_response->getTransId() );
                $order->add_order_note( $complete_message );
                WC_Authnet_API::log( 'Success: ' . $complete_message );
                $order->update_meta_data( '_authnet_charge_captured', 'yes' );
                $order->update_meta_data( 'Authorize.Net Payment ID', $trx_response->getTransId() );
                $order->set_transaction_id( $trx_response->getTransId() );
                $order->save();
            }
    ...
    public function cancel_payment( $order_id )
    {
        ...
        if ( $order->get_payment_method() == 'authnet' ) {
            ...
            if ( $charge ) {
                ...
                $cancel_message = sprintf( __( 'Authorize.Net charge refunded (Charge ID: %s)', 'wc-authnet' ), $trx_response->getTransId() );
